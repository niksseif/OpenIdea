...
router.post('/', (req, res, next) => {
  ...
  console.log(email,password,"<<<email and pass before hashing");
  knex('users')
    .where('email', email)
    .first()
    .then(user => {
      ...
      // user bcrypt.compare to input to hashed password in DB
        let passwordGood = bcrypt.compareSync(user.password, hashed)
        console.log(hashed,user.hashed_password,"<<<<this is comparing pass and hash");
